...
.au-plan-badge { display: inline-flex; align-items: center; gap: 0.5rem; margin-top: 1rem; padding: 0.4rem 1rem; border: 1px solid var(--bd-2); font-size: 9.5px; letter-spacing: 0.16em; text-transform: uppercase; font-weight: 600; color: var(--gold); }
.au-form { display: flex; flex-direction: column; gap: 1.75rem; }
.au-field { position: relative; }
.au-field input { width: 100%; background: transparent; border: none; border-bottom: 1px solid var(--bd); padding: 1.1rem 0 0.5rem 0; font-size: 16px; color: var(--tx); font-family: 'Cabinet Grotesk', sans-serif; outline: none; transition: border-color 0.3s; }
...
.au-field-hint { font-size: 9.5px; letter-spacing: 0.1em; color: var(--tx-3); margin-top: 0.4rem; }
...
.au-google { width: 100%; padding: 0.85rem 1rem; border: 1px solid var(--bd); background: transparent; color: var(--tx-2); font-size: 10px; letter-spacing: 0.18em; text-transform: uppercase; font-weight: 600; font-family: 'Cabinet Grotesk', sans-serif; cursor: pointer; transition: border-color 0.2s, color 0.2s; display: flex; align-items: center; justify-content: center; gap: 0.75rem; min-height: 48px; }
      ...
      <div className="au">
        <div className="au-left">
          ...
          <div className="au-left-body">
            ...
            <h2 className="au-left-title">Comece a<br />flertear com<br /><em>inteligência.</em></h2>
          ...
          <div className="au-form-wrap">
            ...
            <form onSubmit={handleSubmit} className="au-form">
              <div className="au-field">
                <input id="name" type="text" placeholder=" " value={formData.name}
                  onChange={e => setFormData({ ...formData, name: e.target.value })} required autoComplete="name" />
                <label htmlFor="name">Nome completo</label>
              </div>
              <div className="au-field">
                <input id="email" type="email" placeholder=" " value={formData.email}
              ...
              <div className="au-field">
                <input id="password" type="password" placeholder=" " value={formData.password}
                  onChange={e => setFormData({ ...formData, password: e.target.value })} required minLength={6} autoComplete="new-password" />
                <label htmlFor="password">Senha</label>
                <p className="au-field-hint">Mínimo de 6 caracteres</p>
              </div>
            ...
            <p className="au-foot">
              Já tem conta? <Link href="/auth/login" className="au-link">Fazer login</Link>
            </p>
          </div>
        </div>
      </div>
